...
class MoneyApp {
    ...
    public static $numberActiveRows;

    public function __construct($fixedCosts, $flexibleCosts, $spreadsheet) {

        $this->spreadsheet = IOFactory::load('1jan.17aug.Allaccounts.xls');
        $this->fixedCosts = $fixedCosts;
        $this->flexibleCosts = $flexibleCosts;
        $this->numberActiveRows = $spreadsheet->getActiveSheet()->getHighestRow();

        $this->makingSpreadSheet('hellokonnichiwa');

    }

    public function makingSpreadSheet(string $englishJapanese) {
        ...
        foreach($this->flexibleCosts as $target => $category) {
    ...
    $this->makeParsedData($newSpreadsheet, 'konnichiwa');
    ...
    public function makeParsedData($newSpreadsheet, string $greeting) {
        ...
        $this->monthFiltering($results49, '498586642');
    }
    ...
    public function monthFiltering(array $resultsAccount, string $accountNumber) {
        ...
        foreach($months as $month) {
            ...
            $this->monthCalculation($results[$accountNumber][$month], $month);
        }
    }

    function monthCalculation(array $monthlyResults, string $month) {
